...
element_location=zeros(number_elements) %???????? +1 or not?
for f=1:1:number_elements
    element_location(1)=delta_r;
    element_location(f)=element_location(f)+delta_r*f;
end 
...
for j=(number_elements):-1:1 
    ...
    reaction_rate_array(number_elements)=stoichio_H2*k*concentration_H2(number_elements)*f_x ; %reaction rate is never zero on surface only when FeO is all consumed.
    ...
    %the hydrogen concentration at the subsurface element is still zero and waiting to be updated by Gas Phase Mass Balance.
    ...
    if (j>=2)&&(j<=number_elements-2)  %middle
        ...
        f_x=(concentration_FeO(j)/initial_concentration_FeO)^(1/3); %f_x should be calculated at every element, to null the reaction rate as FeO is all consumed.
        ...
        %mass balance of gas species inside the particle. Remember to consider the porosity. 
    ...
    f_x=(concentration_FeO(j)/initial_concentration_FeO)^(1/3);
    ...
    H2_concentration_mesh_element_array(i,j)=concentration_H2(j) 
    FeO_concentration_mesh_element_array(i,j)=concentration_FeO(j)
...
%Data organising
mole_number_FeO=zeros(number_elements,1); % mole nuumber of FeO should be reset at every time step to pass down the data for storage and output.
for m=number_elements:-1:1
    if m>1    
    mole_number_FeO(m)=(4/3*pi*(element_location(m)/1000)^3-4/3*pi*(element_location(m-1)/1000)^3)*concentration_FeO(m);
    ...
    if m==1
    mole_number_FeO(m)=(4/3*pi*(element_location(m)/1000)^3)*concentration_FeO(m);
    mole_consumption_rate=(4/3*pi*(element_location(m)/1000)^3)*reaction_rate_array(m)*(1-porosity)/stoichio_H2*stoichio_FeO;
    end 
end 
...
if rem(i,100)==0||i==total_time_steps
    ...
    mole_number_FeO_scalar=sum(mole_number_FeO(:))
    average_concentration=mole_number_FeO_scalar/(volume)
    consumed_mole_FeO=initial_n_FeO-mole_number_FeO_scalar %scalar, which would be used further for data storage in matrice.
    ...
    mole_number_FeO_array(acquisition_index)=mole_number_FeO_scalar;
